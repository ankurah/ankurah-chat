//! Per-thread read cursors and unread badges for direct messages — the
//! room-cursor pattern of [`crate::read_state`], keyed on a thread.
//!
...
//! trait that would let one manager serve both, so unifying them means
//! designing a read-cursor abstraction over ankurah models — worth doing, and
//! a bigger piece of work than making this one generic.
//!
...
//!
//! Known and accepted: the window is the thread's newest ten messages whoever
//! wrote them, so a badge reads low when the reader's own replies fill it.
//! Room badges undercount the same way for the same reason. The
//! number is an at-a-glance signal that someone is waiting, not an accounting
//! of how many times they said so.
//!
//! Privacy note that does not apply to rooms: these rows are meant to be
//! scoped to their OWNER (`user = $jwt.sub`), not to the thread's participant
//! pair, so a read cursor is never a read receipt the correspondent can see.
...
use ankurah_chat_model::{DmMessageView, DmReadState, DmReadStateView, DmThreadView};
...
/// clock where a deployment settles it (see [`stamp_of`]). So a reader
...
/// every one of its rows ([`super::thread_view`]), so a twin legitimately holds a
...
/// settles it, would otherwise leave a
...
///   on it ([`super::thread_view`]), while the window below filters tombstones
///   out. So
...
/// later than the server's clock is expected to be rewritten to that clock by
/// the server and committed, so what arrives here is honest.
///
...
/// the server owning the number. What must not outlive it is a read cursor
/// set from the claimed value, and
...
impl DmReadStateManager {
    ...
    /// (`recompute_thread`) — both of them the server's number, not this
    ...
    /// on a slow machine. `recompute_thread` holds the cursors down
    ...
    fn recompute_thread(inner: &Arc<Inner>, thread_id: &str) {
        ...
        // `loaded()` is what tells them apart, the same distinction any
        // component draws before it renders "nothing here". Ungated, the wrong
        // reading is the ruinous
    ...
    /// delivered, see `recompute_thread` — because the ceiling is
