...
//! entirely, so markup an author typed into a message never reaches the DOM as
//! markup; plain text nodes are DOM-escaped by Leptos.
//!
...
//!
//! Mention tokens: `<@BASE64_ENTITY_ID>` (the canonical format shared
//! with the server via `ankurah_chat_model::parse_mentions`) render as a
...
impl MentionCtx<'_> {
    /// The highlighted `@name` chip for a mention id. It is a button when the
    /// host offered somewhere for it to lead — a member profile surface — and
    /// an inert span otherwise, rather than a control that does nothing.
    /// Unknown ids (the member is not loaded, or the id is a stranger's)
    /// render as a muted `@unknown` span.
        ...
        match resolved {
            Some((name, entity_id)) if has_detail => {
                let open_detail = move |e: leptos::ev::MouseEvent| {
                    // A host may have installed its own handler on the bubble
            ...
            Some((name, _)) => view! { <span class="mdMention">{format!("@{name}")}</span> }.into_any(),
...
pub fn render_message(text: &str, mention_names: &HashMap<String, String>) -> AnyView {
    // Mentions first: swap each token for a sentinel pair so the
    ...
    let mention_ids = ankurah_chat_model::parse_mentions(text);
    let prepared: String;
