//! The scrolling message timeline, shared by the room log and DM threads.
//!
//! One implementation, not two: the pinned-to-bottom contract is subtle enough
//! — three cooperating effects, a ResizeObserver, and a pixel-truth flag that
//! exists to heal a stranded mode — that a second copy would drift within a
//! release. A room and a DM thread differ only in which collection they page
//! over, so the pane is generic over the view type and knows nothing about
//! either.
//!
//! What the pane owns:
//!
...
//!
//! DOM contract, load-bearing: each row's bubble carries `data-msg-id`, the
//! base64 entity id. The pane finds visible rows by that attribute, and a
//! host's end-to-end tests can find rows the same way.
