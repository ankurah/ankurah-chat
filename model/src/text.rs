//! Pure text scanners shared by every consumer of the chat model — the
//! server workers that act on a message and the clients that render it.
//!
//! All sides must agree byte-for-byte on what counts as a mention token and
//! what counts as a URL: a client writes mention tokens (autocomplete) and
//! looks cached link previews up by URL string equality, while the server
//! parses the same text to fan out notifications and to decide which URLs to
//! unfurl. Disagreement shows up as a message that highlights a mention
//! nobody was notified about, or a preview row nobody can find. Keeping the
//! scanners here — pure functions, no ankurah types, no I/O — in ONE crate
...
/// filler than a shareable link, and downstream consumers (an unfurl worker,
/// the preview rows it writes) shouldn't carry unbounded strings.
...
/// a single message can trigger (a serial fan-out worker probes storage per
...
/// Most URLs one message can emit. Bounds an unfurl worker's per-message
/// fetch work (serial, seconds per URL) and the preview rows it creates.
...
/// no query stripping): it is the key consumers dedup cached link previews by,
