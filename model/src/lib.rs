//! The chat data model: the ankurah collections a chat surface reads and
//! writes, plus the mention/URL scanner every consumer has to agree on.
//!
//! This crate is the half of a chat deployment that server and client share. A
//! durable server node persists these collections and runs workers over them;
//! a client node (native or wasm) queries and writes the same rows. Both link
//! this crate, so there is exactly one definition of what a message is and
//! exactly one definition of what counts as a mention.
//!
//! It is deliberately small — ankurah and serde, no runtime, no UI framework —
//! and it compiles for `wasm32-unknown-unknown` as readily as for a server.
//! Collections a particular deployment invents for itself (moderation records,
...
//! (`Message` → `message`, `DmThread` → `dmthread`) and a property's name from
//! the field name lowercased. Stored rows carry those strings, and a server's
//! access policy names them too. Renaming a struct or a field is therefore a
//! data migration and a policy change, never a refactor: existing rows stop
//! being found, and existing policy entries stop matching the collection they
//! were written for.
//!
//! # Adding a field to a collection that already has rows
//!
//! Such a field must be `Option<T>`. Rows written before it existed carry no
//! such property, and only `Option<T>` reads an absent property as `None` — a
//! bare type surfaces `PropertyError::Missing` instead. Several fields below
//! are `Option` for exactly that reason and say so.
//!
//! Two storage shapes then collapse to the same `None`: a row created with
//! `None` carries the property with a null value (the derive initializes every
//! field), while a row predating the field lacks the key entirely. Same read,
//! different bytes — so **queries touching such a field must stay
//! equality-only**. Rows whose property is absent are skipped by every storage
//! engine, but by different mechanisms: sled and the reactor deny per-row on
//! the absent-property error, whereas IndexedDB excludes them through its
//! composite equality index (null is not a valid key) and would PROPAGATE a
//! per-row error for `!=` or `IN`.
//!
//! That rule governs RETROFITS only. Do not apply it to the direct-message
//! collections' participant fields — making one of those optional hides the
//! row from the very people it belongs to. The direct-messages section below
//! walks through why.
...
pub struct User {
    ...
    /// Stable subject identifier from the OIDC provider (the `sub` claim).
    /// `None` for anonymous users; `Some` once a user signs in. Users are keyed
...
pub struct Room {
    pub name: String,
    /// Creator of the room. `None` for rooms a deployment seeds, which makes
    /// them moderator-managed only under the usual room write scope
...
pub struct Message {
    ...
    /// usual message write scope is `user = $jwt.sub OR collaborative =
    /// true`). `Option<bool>` rather than `bool`: messages predating the field
    /// have no such property, and a bare `bool` read would error with
    /// `PropertyError::Missing` instead of defaulting. Absent/`None` means not
    /// collaborative. Only the author can flip this (a non-author write must
    /// satisfy the scope on the post-write state too, and with `collaborative`
    /// no longer `true` it would not).
    pub collaborative: Option<bool>,
    /// The message this one replies to (nested replies). `None` for ordinary
    /// messages, and absent on every row written before replies existed — only
    ...
    /// so queries touching this field must stay equality-only; the crate docs'
    /// retrofit section says why. Set at creation, never edited.
    #[active_type(LWW)]
...
/// The usual reaction write scope (`user = $jwt.sub`) has no moderator bypass:
/// moderators do not edit other people's reactions.
#[derive(Model, Debug, Serialize, Deserialize)]
pub struct Reaction {
    #[active_type(LWW)]
...
/// usual readstate policy scopes both reads and writes to `user = $jwt.sub`,
/// so these rows are private to their owner.
#[derive(Model, Debug, Serialize, Deserialize)]
...
// ---------------------------------------------------------------------------
// Direct messages (two-party)
// ---------------------------------------------------------------------------
//
// The three `Dm*` collections below all carry their membership ON THE ROW, so
...
// first and propagates an evaluator error before it ever reaches the right
// operand, and comparing against a property an entity does not have IS an
...
// be added to a `Dm*` collection after the fact. A future reader "fixing" the
...
/// storage engines collate `Ref` values by. [`canonical_thread`] breaks the
/// duplicate-thread tie with this same `Ord`, for the same reason.
///
/// A self-pair (`x == y`) is returned unchanged. A chat UI has no reason to
/// offer a self-thread, but the helper is total rather than partial so no
/// caller has to invent an error path for a case it cannot reach.
...
/// write the pair in [`canonical_pair`] order, but the `dmthread` write scope
...
/// it can be pinned by a test — the client that runs it is typically a wasm
/// binary no test in CI compiles, and a typo would surface only as a "Message"
...
/// it lives here rather than in a client so that the race test, the client, and
/// any future consumer converge by construction rather than by coincidence.
///
...
/// every reading path must take the union of the pair's rows; agreeing on where
/// to write must not make what was already written unreachable.
...
pub struct DmThread {
    ...
    pub created_at: i64,
    /// Tombstone flag. **Nothing in this crate writes it**, nor does the
    /// reference consumer — a ruling rather than an omission: a DM rate limiter
    /// tombstones the offending MESSAGE and leaves the conversation standing,
    /// because nothing writes `deleted` back to `false` and an automatic
    /// penalty must not be able to destroy a two-way history it may have
    /// misjudged. There is no user-facing "delete conversation" affordance
    /// either.
    ///
    ...
    /// find-or-create looks for live threads only, so a tombstoned thread would
    /// never be found, a second one would be minted beside it, and the original
    /// history would be unreachable for both participants.
    #[active_type(LWW)]
    pub deleted: bool,
}
...
/// filed under — `thread` — by the render paths, by a DM notification fan-out
...
pub struct DmMessage {
    ...
    #[active_type(LWW)]
    pub b: Ref<User>,
    /// The sender. The usual `dmmessage` write scope pins this to the caller
    ...
    /// renderer and any diff inspector would each need a second code path.
    pub text: String,
    /// ms since epoch (same unit as `Message.timestamp`), written by the
    /// sending client — and, unlike `Message.timestamp`, MEANT TO BE REWRITTEN
    /// BY THE SERVER when it claims a time the server clock has not reached
    /// yet. The reference server settles it once, on first sight, and the whole
    /// DM lane sorts, counts, windows and compares on the settled value.
    ///
    ...
    /// this collection was born with the field, so the `Option` encodes "never
    /// edited", not "the property might be absent". It is not a precedent for
    /// making a participant field optional.
    pub edited_at: Option<i64>,
}

/// Per-user, per-thread read cursor — the `ReadState` pattern, keyed on a
/// thread instead of a room. Unread badges in a DM sidebar are messages newer
...
mod tests {
    ...
    /// false` a client appends.
    ///
    /// Pinned here because the code that runs it is typically a wasm binary CI
    /// never compiles into a test. A typo in that string would not fail a
    /// build; it would fail as a "Message" button that quietly does nothing, on
    /// a path whose whole job is to not open a second thread for a pair.
    #[test]
