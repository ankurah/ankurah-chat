//! Display-form coding for mention tokens: `<@BASE64_ID>` ↔ `@DisplayName`.
//!
//! What this is for: letting a composer show readable names while storing the
//! canonical token. Someone typing `@Ada Lovelace` should see those words in
//! the textarea, and the row that reaches the wire should carry `<@AZk3…>` —
//! the token [`crate::parse_mentions`] matches and a server fans notifications
//! out from. Encode runs once at send; decode runs once when an edit is
//! mirrored back into the textarea.
//!
...
//! side.
//!
//! Nothing here is itself a wire rule. A deployment whose composer types raw
//! tokens never calls this, and its rows are indistinguishable from one whose
//! composer does.
//!
//! Coding rules:
...
/// delimiters, id payload)`. Mirrors the [`crate::parse_mentions`] token rules
/// (base64url payload of 1..=[`MAX_MENTION_ID_LEN`], then `>`), positionally.
